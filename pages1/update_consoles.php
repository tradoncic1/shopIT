<div class="container">
<center><form id="console-form" method="post">
  <input type="text" name="old_name">
  <p><label>Current console name:</label></p><br>
  <br>
  <input type="text" name="name">
  <p><label>Console name:</label></p><br>
  <br>
  <input type="text" name="manufacturer">
  <p><label>Manufacturer:</label></p><br>
  <br>
  <input type="text" name="gpu" required>
  <p><label>GPU:</label></p><br>
  <br>
  <input type="text" name="cpu" required>
  <p><label>CPU:</label></p><br>
  <br>
  <input type="text" name="storage" required>
  <p><label>Storage:</label></p><br>
  <br>
  <input type="number" name="ram" required>
  <p><label>RAM:</label></p><br>
  <br>
  <input type="text" name="price" required>
  <p><label>Price:</label></p><br>
  <br>
  <input type="submit" value="change" name="update_console">
</form></center>
</div>